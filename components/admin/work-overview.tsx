...
export function WorkOverview({ workLogs, employees }: WorkOverviewProps) {
  ...
  const todayLogs = workLogs.filter((log) => log.date === new Date().toISOString().split("T")[0])

  return (
    <Card className="border-slate-700 bg-slate-800 p-6">
      <div className="space-y-6">
        <div>
          <h3 className="text-lg font-semibold text-slate-50 mb-2">Work Overview</h3>
          <div className="flex items-center gap-2 text-sm text-slate-400">
            <Clock className="w-4 h-4" />
            Total work time: {totalHoursFormatted}h {totalMinutesFormatted}m across {todayLogs.length} tasks
          </div>
        </div>
        ...
        <div className="space-y-3 max-h-96 overflow-y-auto">
          {todayLogs.length > 0 ? (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="font-medium text-slate-50 truncate">{employee?.name}</p>
                        <span className="text-xs text-slate-500 bg-slate-800 px-2 py-1 rounded">
                          {employee?.department}
                        </span>
                      </div>
                      <p className="text-sm text-slate-300 mb-2">{log.taskDescription}</p>
                      <p className="text-xs text-slate-400">Logged at {log.createdAt}</p>
                    </div>
            ...
            <p className="text-center text-slate-400 py-8">No work logs recorded yet today</p>
          )}
        </div>
      </div>
    </Card>
